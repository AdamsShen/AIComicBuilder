...
/**
 * 从 DEFAULT_CONTENT 取 zh 值作为默认内容，避免在 registry 里重复维护一份中文。
 * 若 DEFAULT_CONTENT 中无对应条目（开发期漏配），回退到传入的 fallback，
 * 既保持单一来源又防止断档。
 */
function slotFromCanon(
  promptKey: string,
  slotKey: string,
  editable: boolean,
  fallback: string,
): PromptSlot {
  const canonKey = `${promptKey}.${slotKey}`;
  const zh = DEFAULT_CONTENT[canonKey]?.zh;
  if (!zh) {
    console.warn(
      `[registry] DEFAULT_CONTENT 缺少 "${canonKey}"，回退到内联默认值。请补齐 default-content.ts。`,
    );
  }
  return {
    key: slotKey,
    nameKey: `promptTemplates.slots.${camel(slotKey)}`,
    descriptionKey: `promptTemplates.slots.${camel(slotKey)}Desc`,
    defaultContent: zh ?? fallback,
    editable,
  };
}
...
const worldSettingDef: PromptDefinition = {
  ...
  category: "story",
  slots: [
    slotFromCanon("world_setting", "system_prompt", true, "你是一位资深的虚构世界架构师…（见 default-content.ts 的 zh 字段）"),
  ],
  buildFullPrompt(sc) {
    const s = this.slots;
    const r = (k: string) => resolve(sc, s, k);
...
const episodeSummaryDef: PromptDefinition = {
  ...
  category: "story",
  slots: [
    slotFromCanon("episode_summary", "system_prompt", true, "你是漫画剧本的剧情总结助手…（见 default-content.ts 的 zh 字段）"),
  ],
  buildFullPrompt(sc) {
    const s = this.slots;
    const r = (k: string) => resolve(sc, s, k);
...
const canonExtractDef: PromptDefinition = {
  ...
  category: "story",
  slots: [
    slotFromCanon("canon_extract", "system_prompt", true, "你是连载故事的设定集/圣经维护者…（见 default-content.ts 的 zh 字段）"),
  ],
  buildFullPrompt(sc) {
    const s = this.slots;
    const r = (k: string) => resolve(sc, s, k);
...
const aiOptimizeTextDef: PromptDefinition = {
  ...
  category: "story",
  slots: [
    slotFromCanon("ai_optimize_text", "system_prompt_with_image", true, "你是一位专业的AI动画内容优化专家…（见 default-content.ts 的 zh 字段）"),
    slotFromCanon("ai_optimize_text", "system_prompt_without_image", true, "你是一位专业的AI动画内容优化专家…（见 default-content.ts 的 zh 字段）"),
  ],
  buildFullPrompt(sc) {
 ...
 *
 * 单一事实源：zh 默认内容统一取自 DEFAULT_CONTENT（default-content.ts），
 * 避免与 registry 内联字符串两份维护、互相漂移。
 * DEFAULT_CONTENT 缺项时回退到 slot 自带的 defaultContent（安全网）。
 */
export function getDefaultSlotContents(
  ...
  for (const s of def.slots) {
    const fromDefault = (
      DEFAULT_CONTENT[`${def.key}.${s.key}`] as
        | Record<string, string>
        | undefined
    )?.zh;
    result[s.key] = fromDefault ?? s.defaultContent;
  }
  return result;
}
